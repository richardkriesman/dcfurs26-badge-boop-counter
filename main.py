...
BOOP_COUNT_TIMEOUT = 3  # time until another boop can be counted
...
while True:
    ...
    while ival > 0:
        ...
        # Service events.
            ...
            # check boop count timeout
            should_count_boop = False
            with open('boop-last-time.txt', 'r') as f:
                last_boop_time = int(f.read())
                current_time = time.time()
                if (current_time - last_boop_time) >= BOOP_COUNT_TIMEOUT:
                    should_count_boop = True
        
            # boop count stuff
            if should_count_boop:
            
                # update last boop time
                with open('boop-last-time.txt', 'w') as f:
                    f.write(str(time.time()))
            
                # increment boop count
                with open('boop-count.txt', 'r') as f:
                    boop_count = int(f.read())
                boop_count += 1
                with open('boop-count.txt', 'w') as f:
                    f.write(str(boop_count))
                
            # fire on boop handler
            if hasattr(anim, 'on_boop'):
                anim.on_boop()
